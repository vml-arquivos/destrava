...
// CORREÇÃO (11/09/2026 -- documento de identificação do sócio, ex.: CNH real
// da SENATRAN): campos oficiais de documentos de identidade trazem o rótulo
// prefixado por um número de campo ("2 e 1 NOME E SOBRENOME", "5 Nº
// REGISTRO"), então o rótulo nunca está no INÍCIO da linha (como
// `valorAposRotulo` exige) -- apenas CONTIDO nela. Esta função procura o
// rótulo em qualquer posição da linha e devolve a(s) linha(s) seguinte(s),
// que é onde esses formulários oficiais imprimem o valor.
function linhaAposIndiceContendo(linhas: string[], termos: string[], limite = 2): string | null {
  const termosNorm = termos.map(textoNormalizado).sort((a, b) => b.length - a.length);
  const indice = linhas.findIndex((linha) => {
    const normalizada = textoNormalizado(linha);
    return termosNorm.some((termo) => normalizada.includes(termo));
  });
  if (indice === -1) return null;
  for (let offset = 1; offset <= limite && indice + offset < linhas.length; offset += 1) {
    const candidato = limparValor(linhas[indice + offset]);
    if (candidato) return candidato;
  }
  return null;
}
...
function parseComprovanteResidencia(texto: string): { dados: Record<string, any>; confianca: number } {
  ...
  // CORREÇÃO (11/09/2026 -- pedido explícito do usuário: comprovante de
  // endereço ficava preso em "Revisão necessária" mesmo com um documento
  // real e legível): o perfil de análise deste tipo exige `endereco_completo`
  // como campo essencial (`registrarPerfis(['comprovante_residencia'],
  // ['nome_titular', 'endereco_completo', 'data_emissao'], ...)`), mas este
  // leitor nunca extraía esse campo -- o endereço ficava permanentemente
  // ausente, para QUALQUER comprovante de endereço anexado ao sistema,
  // independente da qualidade do arquivo. Corrigido: primeiro tenta um
  // rótulo explícito de endereço completo; se o comprovante traz o endereço
...
// CORREÇÃO (11/09/2026 -- pedido explícito do usuário, com CNH real anexada:
// "mesmo colocando o cnh, o comprovante de endereço, ele está pedindo
// revisão [...] tem que saber ler todo o documento"): documento de
// identificação do sócio (RG/CNH/CPF) nunca teve leitor local dedicado --
// caía no parser genérico, que (a) nunca captura os campos essenciais do
// perfil (`nome`, `numero_documento`), porque os rótulos oficiais desses
// formulários vêm com prefixo numérico de campo ("2 e 1 NOME E SOBRENOME",
// "5 Nº REGISTRO") e nunca estão no início da linha, e (b) captura o CPF
// errado numa CNH real: a regra genérica de CPF pega o primeiro número de
// 11 dígitos do texto, que na CNH é o Nº DE REGISTRO (aparece antes do CPF
...
// Anual e Recibo de entrega reais anexados): a Declaração de IRPF completa
// tinha dois defeitos sistemáticos no parser genérico, reproduzíveis em
// QUALQUER declaração de qualquer sócio -- não específicos desta empresa:
// (1) `ano_calendario` capturava o ano do EXERCÍCIO (sempre o ano seguinte),
// não o ano-calendário de fato exigido pelo perfil, porque "Exercício" vem
// antes de "Ano-calendário" no cabeçalho oficial e a regex genérica aceita
// qualquer um dos dois rótulos, vencendo sempre o primeiro que aparece; (2)
// `titular` capturava lixo de uma linha de tabela de rendimentos que começa
// com a palavra "Titular" (identificador de beneficiário, não o nome do
// declarante), porque a lista de rótulos genérica de "titular" não distingue
// esse uso tabular do rótulo de identificação da declaração -- o campo
// correto é simplesmente o próprio nome do declarante, já lido corretamente
// como `nome`.
function parseDeclaracaoIrpf(texto: string): { dados: Record<string, any>; confianca: number } {
  const base = parseDocumentoGenerico(texto, 'irpf');
  const linhas = linhasTexto(texto);
  const norm = textoNormalizado(texto);
  // A Declaração completa traz "IDENTIFICAÇÃO DO CONTRIBUINTE"; o Recibo de
  // entrega traz "IDENTIFICAÇÃO DO DECLARANTE" e o próprio rótulo "recibo de
  // entrega" -- essa distinção evita aceitar um recibo anexado no campo da
  ...
  const competencia = anoCalendario
    ? { inicio: `${anoCalendario}-01-01`, fim: `${anoCalendario}-12-31` }
    : base.dados.competencia || null;
  const camposComprovados: Record<string, any> = {
    ...(base.dados.campos_comprovados || {}),
    ...(nome ? { nome, titular: nome } : {}),
    ...(cpf ? { cpf } : {}),
    ...(anoCalendario ? { ano_calendario: anoCalendario } : {}),
  };
  const dados = {
    ...base.dados,
    ...camposComprovados,
    documento_compativel: ehDeclaracaoCompleta,
    tipo_detectado_local: ehDeclaracaoCompleta ? 'IRPF' : null,
    titular: nome,
    ano_calendario: anoCalendario,
    competencia,
    campos_comprovados: camposComprovados,
  ...
  const confianca = clamp(base.confianca + (ehDeclaracaoCompleta ? 0.2 : 0) + (nome ? 0.08 : 0) + (anoCalendario ? 0.08 : 0) + (cpf ? 0.05 : 0));
  return { dados, confianca };
}

// CORREÇÃO (11/09/2026 -- mesmo pedido do usuário): o Recibo de entrega da
// Declaração de Ajuste Anual nunca teve leitor local dedicado. Além dos dois
// defeitos do ano-calendário/titular (mesma causa do parser da declaração
// completa, ver comentário acima), o classificador central nunca reconhecia
// "RECIBO_IRPF" como um tipo distinto (só existia a detecção genérica de
// "IRPF"), então um recibo real de entrega SEMPRE era marcado como
// "Documento incompatível" -- o tipo esperado (RECIBO_IRPF) nunca coincidia
// com o único tipo que o classificador sabia detectar (IRPF). A correção do
// classificador está em `classificadorDocumentalCentral.ts`; aqui a leitura
// específica extrai CPF, nome do declarante e o número oficial do recibo
// (formato "XX.XX.XX.XX.XX-XX" impresso pela Receita Federal).
function parseReciboIrpf(texto: string): { dados: Record<string, any>; confianca: number } {
  const base = parseDocumentoGenerico(texto, 'recibo_irpf');
  const norm = textoNormalizado(texto);
  const ehRecibo = /recibo\s+de\s+entrega\s+da\s+declaracao\s+de\s+ajuste\s+anual/.test(norm);
  ...
  const competencia = anoCalendario
    ? { inicio: `${anoCalendario}-01-01`, fim: `${anoCalendario}-12-31` }
    : base.dados.competencia || null;
  // Número oficial do recibo, no formato fixo "XX.XX.XX.XX.XX-XX" impresso
  // pela Receita Federal ("O NÚMERO DO RECIBO [...] é: XX.XX.XX.XX.XX-XX").
  const reciboFormatado = texto.match(/\b(\d{2}\.\d{2}\.\d{2}\.\d{2}\.\d{2}\s*-\s*\d{1,3})\b/)?.[1]?.replace(/\s+/g, '') || null;
  // Protocolo interno do agente receptor (número que acompanha "Declaração
  // recebida via Internet [...] pelo Agente Receptor"), aceito como
  // evidência alternativa quando o número formatado da Receita não aparece.
  const protocoloAgenteReceptor = texto.match(/agente\s+receptor[^\n\d]{0,60}\n?[^\n\d]{0,60}(\d{8,15})/i)?.[1] || null;
  const reciboOuProtocolo = reciboFormatado || protocoloAgenteReceptor || base.dados.recibo_ou_protocolo || null;
  const camposComprovados: Record<string, any> = {
    ...(base.dados.campos_comprovados || {}),
    ...(titular ? { titular } : {}),
    ...(cpf ? { cpf } : {}),
    ...(anoCalendario ? { ano_calendario: anoCalendario } : {}),
    ...(reciboOuProtocolo ? { recibo_ou_protocolo: reciboOuProtocolo } : {}),
  };
  const dados = {
    ...base.dados,
    ...camposComprovados,
    documento_compativel: ehRecibo,
    tipo_detectado_local: ehRecibo ? 'RECIBO_IRPF' : null,
    ano_calendario: anoCalendario,
    competencia,
    recibo_ou_protocolo: reciboOuProtocolo,
    campos_comprovados: camposComprovados,
    fonte_extracao: 'local_deterministica_especializada',
  };
  const confianca = clamp(base.confianca + (ehRecibo ? 0.2 : 0) + (reciboOuProtocolo ? 0.1 : 0) + (anoCalendario ? 0.08 : 0) + (cpf ? 0.05 : 0) + (titular ? 0.05 : 0));
  return { dados, confianca };
}
...
export function analisarTextoDocumentoLocal(tipo: TipoDocumentoLocal, texto: string, tipoDocumentoEsperado?: string): { dados: Record<string, any>; confianca: number } {
  ...
  if (tipo === 'documento_identidade_socio') return parseDocumentoIdentidadeSocio(texto);
  if (tipo === 'declaracao_irpf') return parseDeclaracaoIrpf(texto);
  if (tipo === 'recibo_irpf') return parseReciboIrpf(texto);
