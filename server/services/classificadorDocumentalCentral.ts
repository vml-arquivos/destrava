...
function detectarTipo(texto: string): { tipo: TipoDetectadoDocumental; evidencias: string[]; confianca: number } {
  ...
  // CORREÇÃO (11/09/2026, pedido explícito do usuário com um Recibo de
  // entrega real anexado): faltava esta detecção -- o Recibo de entrega da
  // Declaração de Ajuste Anual (IRPF) sempre caía na regra genérica de IRPF
  // logo abaixo, e como o campo espera o tipo "RECIBO_IRPF" (distinto do
  // tipo "IRPF" da declaração completa, no mesmo espírito de RECIBO_ECF x
  // ECF e RECIBO_PGDAS x PGDAS-D acima), a comparação nunca batia e QUALQUER
  // recibo de entrega real era marcado "Documento incompatível", sempre.
  const temReciboIrpf = /recibo\s+de\s+entrega\s+da\s+declaracao\s+de\s+ajuste\s+anual/.test(n);
  ...
  if (temReciboIrpf) { push('recibo IRPF'); return { tipo: 'RECIBO_IRPF', evidencias, confianca: 0.97 }; }
