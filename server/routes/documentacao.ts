...
import {
  ...
  linhaObjetivaDocumento,
  nomeFuncionalDocumento,
...
function gerarHtmlRelatorioDocumentalTecnico(relatorio: any): string {
  ...
  const analisadosHtml = analisados.length ? analisados.map((documento: any) => {
    ...
    const linhaObjetiva = linhaObjetivaDocumento(resultado, documento, nomeFuncional, rotuloEstadoDocumento(estado));
    return `<article class="doc ${classeDocumento}"><div class="doc-head"><div><strong>${escapeHtmlRelatorio(nomeFuncional)}</strong><small>Arquivo original: ${escapeHtmlRelatorio(documento.nome || documento.nome_original || 'não localizado')} · ${escapeHtmlRelatorio(documento.bloco)}${documento.criado_em ? ` · recebido em ${escapeHtmlRelatorio(dataRelatorio(documento.criado_em))}` : ''}</small></div><span class="pill ${classePill}">${escapeHtmlRelatorio(rotuloEstadoDocumento(estado))}</span></div><p class="objective-line">${escapeHtmlRelatorio(linhaObjetiva)}</p>${secoesAnaliseHtml(resultado, documento)}</article>`;
  ...
  const pendentesAnaliseHtml = pendentesAnalise.length ? pendentesAnalise.map((documento: any) => {
    const resultado = documento.resultado_analise || {};
    const nomeFuncional = nomeFuncionalDocumento(documento);
    const linhaObjetiva = linhaObjetivaDocumento(resultado, { ...documento, analisado: false }, nomeFuncional, 'Não analisado');
    return `<article class="doc waiting"><div class="doc-head"><div><strong>${escapeHtmlRelatorio(nomeFuncional)}</strong><small>Arquivo original: ${escapeHtmlRelatorio(documento.nome || documento.nome_original || 'não localizado')} · ${escapeHtmlRelatorio(documento.bloco)}${documento.criado_em ? ` · recebido em ${escapeHtmlRelatorio(dataRelatorio(documento.criado_em))}` : ''}</small></div><span class="pill orange">Aguardando análise</span></div><p class="objective-line">${escapeHtmlRelatorio(linhaObjetiva)}</p></article>`;
  ...
  const inventarioInicialHtml = inventarioInicial.length ? `<table><thead><tr><th>Documento funcional / arquivo</th><th>Esperado/recebido</th><th>Leitura</th><th>Resultado e status</th><th>Páginas</th><th>Pendência</th></tr></thead><tbody>${inventarioInicial.map((item: any) => `<tr><td><b>${escapeHtmlRelatorio(item.documento || nomeFuncionalDocumento(item))}</b><small>Arquivo original: ${escapeHtmlRelatorio(item.arquivo || 'não localizado')} · ${escapeHtmlRelatorio(item.etapa || '')}</small></td><td>${item.esperado ? 'Sim' : 'Não'} / ${item.recebido ? 'Sim' : 'Não'}</td><td>${item.lido ? 'Concluída' : 'Não analisada'}</td><td>${escapeHtmlRelatorio(item.linha_objetiva || `${item.documento || 'Documento'} — ${item.resultado_objetivo || item.status || 'Resultado não localizado'}`)}</td><td>${escapeHtmlRelatorio(item.evidencia?.paginas_processadas ?? 'Não localizado')}</td><td>${escapeHtmlRelatorio(item.pendencia || 'Nenhuma')}</td></tr>`).join('')}</tbody></table>` : `<p class="empty">Nenhum item foi inventariado.</p>`;
...
router.get('/empresa/:empresaId/relatorio/pdf', auth, async (req: Request, res: Response) => {
  try {
    ...
    if (!dossie) { res.status(404).json({ error: 'Empresa não encontrada' }); return; }
    const relatorio = await montarRelatorioDocumental(dossie, modo);
    const pdf = await generateBrandedPdfBuffer(gerarHtmlRelatorioModular(relatorio), {
      brand: 'destrava',
      topMargin: '22mm',
      bottomMargin: '18mm',
      leftMargin: '17mm',
