...
function assertIdentificadorDb(value: string, tipo = "identificador"): string {
...
async function tabelaPublicaExiste(client: any, tableName: string): Promise<boolean> {
  assertIdentificadorDb(tableName, "tabela");
...
async function colunasTabelaPublica(client: any, tableName: string): Promise<Set<string>> {
  assertIdentificadorDb(tableName, "tabela");
  const { rows } = await client.query(
    `SELECT column_name
       FROM information_schema.columns
...
async function apagarPorColunaSeExistir(client: any, tableName: string, columnName: string, value: unknown): Promise<number> {
  assertIdentificadorDb(tableName, "tabela");
  assertIdentificadorDb(columnName, "coluna");
  if (!(await tabelaPublicaExiste(client, tableName))) return 0;
  const columns = await colunasTabelaPublica(client, tableName);
  if (!columns.has(columnName)) return 0;
  const r = await client.query(`DELETE FROM public."${tableName}" WHERE "${columnName}" = $1`, [value]);
  return r.rowCount || 0;
}

async function zerarColunaSeExistir(client: any, tableName: string, columnName: string, value: unknown): Promise<number> {
  assertIdentificadorDb(tableName, "tabela");
  assertIdentificadorDb(columnName, "coluna");
  if (!(await tabelaPublicaExiste(client, tableName))) return 0;
  const columns = await colunasTabelaPublica(client, tableName);
  if (!columns.has(columnName)) return 0;
  const r = await client.query(`UPDATE public."${tableName}" SET "${columnName}" = NULL WHERE "${columnName}" = $1`, [value]);
  return r.rowCount || 0;
}

function normalizarCaminhoArquivoParaRemocao(raw: unknown): string | null {
  ...
  if (value.startsWith("/uploads/")) {
    resolved = path.join(dataDir, value.replace(/^\/+/, ""));
  } else if (value.startsWith("uploads/")) {
    resolved = path.join(dataDir, value);
  ...
  const allowedRoots = [
    ...
    path.normalize(process.env.UPLOADS_DIR || path.join(dataDir, "uploads")),
  ];

  if (!allowedRoots.some((root) => normalized === root || normalized.startsWith(root + path.sep))) {
    console.warn("[DELETE empresa] caminho físico ignorado fora de DATA_DIR:", normalized);
    return null;
  }
  return normalized;
}

function adicionarCaminhosDeLinha(paths: Set<string>, row: Record<string, unknown>): void {
  const keys = ["caminho_arquivo", "caminho", "file_path", "pdf_path", "url", "url_arquivo"];
  for (const key of keys) {
    const parsed = normalizarCaminhoArquivoParaRemocao(row?.[key]);
    if (parsed) paths.add(parsed);
  }
}

async function removerArquivosFisicosEmpresa(empresaId: string, caminhos: string[]): Promise<void> {
  const dataDir = process.env.DATA_DIR || "/var/data/destrava";
  const alvos = new Set<string>(caminhos.filter(Boolean));
  alvos.add(path.join(dataDir, "uploads", "empresas", empresaId));
  for (const alvo of alvos) {
    try {
      await fs.promises.rm(alvo, { recursive: true, force: true });
    } catch (err: any) {
      console.warn("[DELETE empresa] falha ao remover arquivo/diretório:", alvo, err?.message || err);
    }
  }
}

async function idsSociosDaEmpresa(client: any, empresaId: string): Promise<string[]> {
  if (!(await tabelaPublicaExiste(client, "socios_empresa"))) return [];
  const columns = await colunasTabelaPublica(client, "socios_empresa");
  if (!columns.has("empresa_id") || !columns.has("id")) return [];
  const { rows } = await client.query(`SELECT id FROM public.socios_empresa WHERE empresa_id = $1`, [empresaId]);
  return rows.map((r: { id: string }) => r.id).filter(Boolean);
}

async function idsDocumentosDaEmpresa(client: any, empresaId: string, socioIds: string[], caminhos: Set<string>): Promise<string[]> {
  if (!(await tabelaPublicaExiste(client, "documentos_arquivos"))) return [];
  const columns = await colunasTabelaPublica(client, "documentos_arquivos");
  if (!columns.has("id")) return [];

  const clauses: string[] = [];
  const params: any[] = [];
  if (columns.has("empresa_id")) { params.push(empresaId); clauses.push(`empresa_id = $${params.length}`); }
  if (columns.has("entidade_tipo") && columns.has("entidade_id")) { params.push(empresaId); clauses.push(`(entidade_tipo = 'empresa' AND entidade_id = $${params.length})`); }
  if (socioIds.length && columns.has("socio_id")) { params.push(socioIds); clauses.push(`socio_id = ANY($${params.length}::uuid[])`); }
  if (!clauses.length) return [];

  const selectCols = ["id", "caminho_arquivo", "url_arquivo", "nome_arquivo"].filter((c) => columns.has(c));
  const { rows } = await client.query(
    `SELECT ${selectCols.map((c) => `"${c}"`).join(", ")} FROM public.documentos_arquivos WHERE ${clauses.join(" OR ")}`,
    params
  );
  for (const row of rows) adicionarCaminhosDeLinha(caminhos, row);
  return rows.map((r: { id: string }) => r.id).filter(Boolean);
}

async function coletarCaminhosLegadosEmpresa(client: any, empresaId: string, caminhos: Set<string>): Promise<void> {
  const tabelas = [
    { table: "empresa_documentos", columns: ["url"] },
    { table: "documentos_empresa", columns: ["url_arquivo", "caminho_arquivo"] },
    { table: "empresas_contratos_sociais", columns: ["caminho_arquivo", "url"] },
  ];
  for (const cfg of tabelas) {
    if (!(await tabelaPublicaExiste(client, cfg.table))) continue;
    const cols = await colunasTabelaPublica(client, cfg.table);
    if (!cols.has("empresa_id")) continue;
    const selectCols = cfg.columns.filter((c) => cols.has(c));
    if (!selectCols.length) continue;
    const { rows } = await client.query(
      `SELECT ${selectCols.map((c) => `"${c}"`).join(", ")} FROM public."${cfg.table}" WHERE empresa_id = $1`,
      [empresaId]
    );
    for (const row of rows) adicionarCaminhosDeLinha(caminhos, row);
  }
...
async function limparAnalisesDocumentosEmpresa(client: any, empresaId: string): Promise<{ caminhos: string[]; documentos: number; analises: number }> {
  const caminhos = new Set<string>();
  const socioIds = await idsSociosDaEmpresa(client, empresaId);
  await coletarCaminhosLegadosEmpresa(client, empresaId, caminhos);
  const documentoIds = await idsDocumentosDaEmpresa(client, empresaId, socioIds, caminhos);
  let analises = 0;
  let documentos = documentoIds.length;

  // Limpeza explícita das camadas de IA/análises para não manter laudos antigos ou falsos positivos.
  analises += await apagarPorColunaSeExistir(client, "analises_cnpj_empresa", "empresa_id", empresaId);
  analises += await apagarPorColunaSeExistir(client, "documentacao_analises_ia", "empresa_id", empresaId);
  analises += await apagarPorColunaSeExistir(client, "documentos_alertas_ia", "empresa_id", empresaId);
  analises += await apagarPorColunaSeExistir(client, "documentos_campos_extraidos", "empresa_id", empresaId);
  analises += await apagarPorColunaSeExistir(client, "documentos_rag_chunks", "empresa_id", empresaId);
  analises += await apagarPorColunaSeExistir(client, "documentos_textos_extraidos", "empresa_id", empresaId);
  analises += await apagarPorColunaSeExistir(client, "documentacao_entidade_blocos", "empresa_id", empresaId);
  analises += await apagarPorColunaSeExistir(client, "credenciais_sensiveis_empresa", "empresa_id", empresaId);

  if (documentoIds.length) {
    // FKs com ON DELETE RESTRICT precisam ser removidas antes do arquivo central.
    if (await tabelaPublicaExiste(client, "documentacao_bloco_arquivos")) {
      await client.query(`DELETE FROM public.documentacao_bloco_arquivos WHERE arquivo_id = ANY($1::uuid[])`, [documentoIds]);
    }
    if (await tabelaPublicaExiste(client, "documentos_extracoes_ia")) {
      await client.query(`DELETE FROM public.documentos_extracoes_ia WHERE arquivo_id = ANY($1::uuid[])`, [documentoIds]);
    }
    if (await tabelaPublicaExiste(client, "auditoria_documentacao")) {
      await client.query(`DELETE FROM public.auditoria_documentacao WHERE arquivo_id = ANY($1::uuid[])`, [documentoIds]);
    }
    await client.query(`DELETE FROM public.documentos_arquivos WHERE id = ANY($1::uuid[])`, [documentoIds]);
  }

  documentos += await apagarPorColunaSeExistir(client, "empresa_documentos", "empresa_id", empresaId);
  documentos += await apagarPorColunaSeExistir(client, "documentos_empresa", "empresa_id", empresaId);
  documentos += await apagarPorColunaSeExistir(client, "empresas_contratos_sociais", "empresa_id", empresaId);

  // Desvincula registros operacionais que precisam permanecer por histórico comercial/contratual.
  await zerarColunaSeExistir(client, "simulacoes_colaborador", "empresa_id", empresaId);
  await zerarColunaSeExistir(client, "contratos_gerados", "empresa_id", empresaId);
  await zerarColunaSeExistir(client, "acompanhamentos_bancarios", "empresa_id", empresaId);
  await zerarColunaSeExistir(client, "triagem_leads", "empresa_id", empresaId);
  await zerarColunaSeExistir(client, "clientes_pf", "empresa_id", empresaId);

  return { caminhos: Array.from(caminhos), documentos, analises };
}

async function resolverFksEmpresaAntesDeExcluir(client: any, empresaId: string): Promise<{ apagados: number; desvinculados: number }> {
  const { rows } = await client.query(
    `SELECT
        tc.table_name,
        kcu.column_name,
  ...
  const jaTratadas = new Set([
    "analises_cnpj_empresa.empresa_id",
    "documentacao_analises_ia.empresa_id",
    "documentacao_entidade_blocos.empresa_id",
    "documentos_campos_extraidos.empresa_id",
    "documentos_rag_chunks.empresa_id",
    "documentos_textos_extraidos.empresa_id",
    "credenciais_sensiveis_empresa.empresa_id",
    "empresa_documentos.empresa_id",
    "documentos_empresa.empresa_id",
    "empresas_contratos_sociais.empresa_id",
    "simulacoes_colaborador.empresa_id",
    "contratos_gerados.empresa_id",
    "acompanhamentos_bancarios.empresa_id",
    "triagem_leads.empresa_id",
    "clientes_pf.empresa_id",
  ]);

  for (const row of rows) {
    const table = assertIdentificadorDb(row.table_name, "tabela");
    const column = assertIdentificadorDb(row.column_name, "coluna");
    if (jaTratadas.has(`${table}.${column}`)) continue;
    if (row.is_nullable === "YES") {
      const r = await client.query(`UPDATE public."${table}" SET "${column}" = NULL WHERE "${column}" = $1`, [empresaId]);
      desvinculados += r.rowCount || 0;
    } else {
      const r = await client.query(`DELETE FROM public."${table}" WHERE "${column}" = $1`, [empresaId]);
      apagados += r.rowCount || 0;
    }
  }

  return { apagados, desvinculados };
}
...
async function startServer() {
  ...
  app.delete("/api/empresas/:id", auth, async (req: Request, res: Response) => {
    ...
    let caminhosParaRemover: string[] = [];
    try {
      const colaborador = (req as Request & { colaborador: any }).colaborador;
      if (!colaboradorPodeVerTudo(colaborador)) {
      ...
      try {
        ...
        const { rows: empresaRows } = await client.query("SELECT id, razao_social, cnpj FROM empresas WHERE id = $1 FOR UPDATE", [empresaId]);
        ...
        const limpeza = await limparAnalisesDocumentosEmpresa(client, empresaId);
        caminhosParaRemover = limpeza.caminhos;
        const fks = await resolverFksEmpresaAntesDeExcluir(client, empresaId);

        const del = await client.query("DELETE FROM empresas WHERE id = $1 RETURNING id", [empresaId]);
        if (!del.rows.length) {
          await client.query("ROLLBACK");
          res.status(404).json({ error: "Empresa não encontrada" });
          return;
        ...
        // Arquivos físicos são apagados somente depois do COMMIT para não perder arquivo se a transação falhar.
        await removerArquivosFisicosEmpresa(empresaId, caminhosParaRemover);

        res.json({
          success: true,
          message: "Empresa excluída com limpeza de documentos, análises IA e vínculos operacionais.",
          limpeza: {
            documentos_removidos: limpeza.documentos,
            analises_removidas: limpeza.analises,
            vinculos_apagados: fks.apagados,
            vinculos_desvinculados: fks.desvinculados,
          },
        });
      } catch (err) {
      ...
      console.error("[DELETE /api/empresas/:id]", { message: err?.message, code: err?.code, detail: err?.detail, constraint: err?.constraint, table: err?.table });
      res.status(500).json({ error: "Erro ao excluir empresa", detalhe: err?.detail || err?.message || "Falha interna" });
    }
  });
