...
function normalizeNumeric(value: unknown): number | null {
  ...
  const original = value.replace(/R\$/gi, "").replace(/\s/g, "").trim();
  ...
  if (s.includes(".")) {
    ...
    // Postgres/APIs retornam NUMERIC como string decimal: 50000.00.
    // Não pode remover o ponto, senão vira 5.000.000,00.
