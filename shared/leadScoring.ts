...
export function calcularScoreBasico(lead: LeadScoreInput): number {
  ...
  if (valor > 0) {
    const logScore = Math.min(
      30,
      Math.round((Math.log10(valor) / Math.log10(5_000_000)) * 30)
    );
  ...
  const temperatura: Record<string, number> = {
    frio: 0,
    morno: 8,
    quente: 15,
    urgente: 20,
...
export function calcularScoreEfetivo(
  lead: LeadScoreInput & {
  ...
  if (lead.score_manual !== null && lead.score_manual !== undefined)
    return Number(lead.score_manual) || 0;
  if (
    lead.score_ia !== null &&
    lead.score_ia !== undefined &&
    Number(lead.score_ia) > 0
  )
    return Number(lead.score_ia);
  if (lead.score_basico !== null && lead.score_basico !== undefined)
    return Number(lead.score_basico) || 0;
