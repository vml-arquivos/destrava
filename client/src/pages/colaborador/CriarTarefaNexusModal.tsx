...
const inputClass = 'w-full border border-input rounded-xl px-3 py-2.5 text-sm bg-card focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary disabled:bg-muted disabled:text-muted-foreground';
      ...
      <div className="w-full max-w-5xl max-h-[96vh] bg-card rounded-2xl shadow-2xl flex flex-col overflow-hidden border border-primary/20">
        ...
        <div className="p-5 overflow-y-auto space-y-4">
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            <div><label className="block text-xs font-bold text-muted-foreground mb-1">Prazo da lista</label><input type="date" className={inputClass} value={prazo} onChange={event => setPrazo(event.target.value)} /></div>
            <div><label className="block text-xs font-bold text-muted-foreground mb-1">Prioridade</label><select className={inputClass} value={prioridade} onChange={event => setPrioridade(event.target.value as typeof prioridade)}><option value="baixa">Baixa</option><option value="media">Média</option><option value="alta">Alta</option></select></div>
          </div>
          ...
          <section>
            <div className="flex items-center justify-between gap-3 mb-2">
              <div><h3 className="font-black text-sm text-foreground flex items-center gap-2"><CheckSquare className="w-4 h-4" /> Checklist</h3><p className="text-[11px] text-muted-foreground">Responsável, data, pontuação e frequência pertencem a cada item e nunca são misturados.</p></div>
              <button type="button" onClick={() => setItems(current => [...current, newChecklistItem()])} className="flex items-center gap-1 text-xs font-bold px-3 py-2 rounded-lg bg-primary/10 text-primary hover:bg-primary/20"><Plus className="w-3.5 h-3.5" /> Nova ação</button>
            </div>
            <div className="space-y-3">
              {items.map((item, index) => {
                const availableMembers = membersForItem(item);
                return (
                  <div key={item.id} className="rounded-2xl border border-border p-3.5 bg-muted shadow-sm">
                    <div className="flex gap-2.5 items-start">
                      <span className="w-7 h-7 rounded-full bg-primary text-primary-foreground text-xs font-black flex items-center justify-center shrink-0">{index + 1}</span>
                      <div className="flex-1 grid gap-2.5 min-w-0">
                        ...
                        <div className="grid md:grid-cols-2 gap-2.5">
                          <label><span className="block text-[11px] font-bold text-muted-foreground mb-1">Data desta ação</span><span className="relative block"><Calendar className="absolute left-3 top-2.5 w-4 h-4 text-muted-foreground" /><input type="date" className={`${inputClass} pl-9`} value={item.data} onChange={event => updateItem(item.id, 'data', event.target.value)} /></span></label>
                          <label><span className="block text-[11px] font-bold text-muted-foreground mb-1 flex items-center gap-1"><Trophy className="w-3.5 h-3.5 text-warning" /> Pontuação oficial do Nexus</span><select className={inputClass} value={item.dificuldade} onChange={event => updateItem(item.id, 'dificuldade', event.target.value as ChecklistDifficulty)}>{DIFFICULTIES.map(option => <option key={option.value} value={option.value}>{option.label} · {option.points} pts</option>)}</select></label>
                        </div>

                        <div className="rounded-xl border border-border bg-card p-3">
                          <div className="flex items-center gap-2 mb-2"><Users className="w-4 h-4 text-primary" /><strong className="text-xs text-foreground">Quem executa esta ação?</strong></div>
                          <div className="grid md:grid-cols-2 gap-2.5">
                            <label><span className="block text-[11px] font-bold text-muted-foreground mb-1">Equipe para filtrar</span><select className={inputClass} value={item.equipe_id} onChange={event => updateItemTeam(item, event.target.value)} disabled={catalogLoading}><option value="">Todas as equipes e pessoas</option>{catalog.equipes.map(team => <option key={team.id} value={team.id}>{team.nome} · {team.membro_ids.length} pessoa(s)</option>)}</select></label>
                            <label><span className="block text-[11px] font-bold text-muted-foreground mb-1">Membro responsável</span><select className={inputClass} value={item.responsavel_id} onChange={event => updateItem(item.id, 'responsavel_id', event.target.value)} disabled={catalogLoading}><option value="">Livre / responsável principal da lista</option>{availableMembers.map(member => <option key={member.id} value={member.id}>{member.nome} · {ROLE_LABELS[member.role] || member.role}{member.cargo ? ` · ${member.cargo}` : ''}</option>)}</select></label>
                          </div>
                          {!catalogLoading && item.equipe_id && availableMembers.length === 0 && <p className="text-[11px] text-warning mt-2">Essa equipe não possui perfil ativo. Escolha outra equipe ou “Todas”.</p>}
                        </div>
        ...
        <footer className="px-5 py-4 border-t flex justify-end gap-2 bg-card shadow-[0_-8px_24px_rgba(15,23,42,0.06)]">
          <button type="button" onClick={onClose} className="px-4 py-2.5 rounded-xl border text-sm font-bold text-muted-foreground hover:bg-muted">Cancelar</button>
          <button type="button" onClick={() => void submit()} disabled={saving} className="px-4 py-2.5 rounded-xl bg-primary hover:bg-primary/90 text-primary-foreground text-sm font-bold flex items-center gap-2 disabled:opacity-60 shadow-lg shadow-blue-700/20">{saving ? <Loader2 className="w-4 h-4 animate-spin" /> : <CheckSquare className="w-4 h-4" />}{saving ? 'Enviando…' : 'Criar no Nexus'}</button>
        </footer>
      </div>
    </div>
  );
