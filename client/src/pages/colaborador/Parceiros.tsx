...
export default function ParceirosPage() {
  ...
  async function visualizarFicha(parceiro: Parceiro) {
    setGerandoFichaId(parceiro.id);
    setFichaPreviewId(parceiro.id);
    setFichaPreviewTitle(`Ficha Cadastral — ${parceiro.nome}`);
    setFichaPreviewHtml(null);
    setFichaPreviewOpen(true);
    try {
      const preview = await apiFetch(`/api/parceiros/${parceiro.id}/ficha/preview`);
      if (!preview?.html) throw new Error("A visualização da ficha não foi disponibilizada.");
      setFichaPreviewTitle(preview.title || `Ficha Cadastral — ${parceiro.nome}`);
  ...
  async function baixarFicha() {
    ...
    try {
      const { blob, filename, contentType } = await apiFetchBlob(`/api/parceiros/${fichaPreviewId}/ficha/pdf`);
      if (!contentType?.toLowerCase().includes("application/pdf")) throw new Error("O servidor não retornou um PDF válido.");
      const url = URL.createObjectURL(blob);
      const anchor = document.createElement("a");
      anchor.href = url;
      anchor.download = filename || `ficha-parceiro-${fichaPreviewTitle.replace(/[^a-zA-Z0-9]/g, "-").toLowerCase()}.pdf`;
      anchor.click();
