...
const STATUS_CONFIG: Record<string, { label: string; cor: string; bg: string; icone: React.ElementType }> = {
  apto:             { label: "Apto ao crédito",     cor: "text-success", bg: "bg-success/10 border-success/20", icone: ShieldCheck },
  pendencias_leves: { label: "Pendências leves",    cor: "text-warning",   bg: "bg-warning/10 border-warning/20",     icone: ShieldAlert },
  pendencias_graves:{ label: "Pendências graves",   cor: "text-warning",  bg: "bg-warning/10 border-warning/20",   icone: ShieldAlert },
  inapto:           { label: "Inapto — ação urgente", cor: "text-destructive",   bg: "bg-destructive/10 border-destructive/20",         icone: ShieldX },
  nao_analisado:    { label: "Não analisado",       cor: "text-muted-foreground",   bg: "bg-muted border-border",     icone: ShieldAlert },
};
...
export default function DiagnosticoCredito() {
  ...
  return (
    <Layout title="Diagnóstico de Crédito">
      <div className="h-full overflow-y-auto bg-muted p-4 lg:p-6">
        <div className="max-w-6xl mx-auto space-y-4">

          {/* Header */}
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              <div className="h-12 w-12 rounded-2xl bg-primary flex items-center justify-center shadow-lg shadow-blue-100">
                <ClipboardCheck className="h-6 w-6 text-primary-foreground" />
              </div>
              <div>
                <h1 className="text-xl font-black text-foreground">Diagnóstico de Crédito</h1>
                <p className="text-xs text-muted-foreground">Visão consolidada da aptidão de todas as empresas</p>
              </div>
            </div>
            <Link href="/colaborador/assessoria">
              <button className="inline-flex items-center gap-2 rounded-2xl bg-success px-4 py-2.5 text-xs font-bold text-primary-foreground hover:bg-success shadow-sm">
              ...
              return (
                <button
                  ...
                  className={`rounded-2xl border p-3 text-center transition-all ${
                    filtroStatus === key ? conf.bg + " ring-2 ring-offset-1" : "bg-card border-border hover:border-input"
                  }`}
                >
                  <Icon className={`h-5 w-5 mx-auto mb-1 ${conf.cor}`} />
                  <div className={`text-xl font-black ${conf.cor}`}>{count}</div>
                  <div className="text-[10px] text-muted-foreground font-semibold leading-tight mt-0.5">{conf.label}</div>
                </button>
              );
            })}
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-2">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full pl-9 pr-3 h-10 rounded-2xl border border-border bg-card text-sm outline-none focus:border-primary/30 focus:ring-1 focus:ring-blue-100"
              />
            </div>
            <button onClick={carregar} className="h-10 w-10 rounded-2xl border border-border bg-card flex items-center justify-center hover:bg-muted shrink-0">
              <RefreshCw className={`h-4 w-4 text-muted-foreground ${carregando ? "animate-spin" : ""}`} />
            </button>
          </div>
                ...
                {filtradas.length === 0 ? (
                  <div className="py-16 text-center text-sm text-muted-foreground">Nenhuma empresa encontrada</div>
                    ...
                    .map(emp => {
                      ...
                      const corScore = score >= 70 ? "text-success" : score >= 50 ? "text-warning" : score >= 30 ? "text-warning" : "text-destructive";
                      return (
                        <Link key={emp.id} href={`/colaborador/assessoria`}>
                          <div
                            onClick={() => {/* abre assessoria com empresa selecionada */}}
                            className="flex items-center gap-4 px-4 py-3 hover:bg-muted transition-all cursor-pointer"
                          >
                            <div className="h-9 w-9 rounded-xl bg-muted flex items-center justify-center font-black text-muted-foreground text-sm shrink-0">
                              {(emp.razao_social || "?").charAt(0)}
                            </div>
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2">
                                <p className="text-sm font-bold text-foreground truncate">{emp.razao_social || emp.nome_fantasia}</p>
                                {emp.porte && <span className="text-[10px] font-semibold text-muted-foreground bg-muted px-1.5 py-0.5 rounded-full shrink-0">{emp.porte}</span>}
                              </div>
                              <p className="text-xs text-muted-foreground truncate">{emp.cnpj}</p>
                            </div>
          ...
          {filtradas.length > 0 && (
            <p className="text-center text-xs text-muted-foreground">{filtradas.length} empresa{filtradas.length !== 1 ? "s" : ""} · {filtroStatus !== "todos" ? STATUS_CONFIG[filtroStatus]?.label : "todos os status"}</p>
          )}
        </div>
      </div>
    </Layout>
