...
export default function CreditoPessoaFisica() {
  return (
    <div className="min-h-screen flex flex-col bg-card">
      ...
      <section className="py-14 bg-card">
        <div className="container px-4">
          <div className="max-w-5xl mx-auto grid lg:grid-cols-[1.35fr_0.65fr] gap-10 items-start">
            <div>
              <p className="text-sm font-bold uppercase tracking-wider text-primary mb-3">Crédito com decisão consciente</p>
              <h2 className="text-3xl md:text-4xl font-bold text-foreground mb-5">A modalidade certa começa pela finalidade, não pela parcela</h2>
            ...
            <aside className="bg-primary/10 border border-primary/20 rounded-2xl p-6">
              <Scale className="h-8 w-8 text-primary mb-4" />
              <h3 className="text-xl font-bold text-foreground mb-4">Quatro perguntas antes de avançar</h3>
              <ol className="space-y-4 text-sm text-foreground">
                {[
                  ...
                  <li key={item} className="flex gap-3">
                    <span className="w-7 h-7 rounded-full bg-primary text-primary-foreground flex items-center justify-center font-bold flex-shrink-0">{index + 1}</span>
        ...
        <div className="container px-4">
          <div className="max-w-5xl mx-auto">
            <div className="text-center mb-12">
              <h2 className="text-3xl font-bold text-foreground mb-4">Nossas Linhas de Crédito</h2>
              <p className="text-muted-foreground">Soluções para cada necessidade</p>
            </div>
            ...
            <div className="grid md:grid-cols-2 gap-6 mb-8">
              {linhasPF.filter((l) => l.destaque).map((linha) => (
                <div key={linha.id} className={`bg-gradient-to-br ${linha.cor} text-primary-foreground rounded-2xl p-8 shadow-lg`}>
                  ...
                  <div className="mb-6">
                    <p className="text-sm font-semibold text-primary-foreground/80 mb-2">Benefícios:</p>
                    <ul className="space-y-1">
                      {linha.beneficios.slice(0, 3).map((b) => (
                        <li key={b} className="flex items-center gap-2 text-xs text-primary-foreground/80">
            ...
            <div className="grid md:grid-cols-2 gap-6">
              {linhasPF.filter((l) => !l.destaque).map((linha) => (
                <div key={linha.id} className="bg-card rounded-2xl p-6 shadow-sm border border-border">
                  <div className="flex items-start gap-4 mb-4">
                    <div className="w-12 h-12 rounded-xl bg-muted flex items-center justify-center flex-shrink-0">
                      <linha.icon className="h-6 w-6 text-muted-foreground" />
                    </div>
                    <div>
                      <Badge className={linha.badgeColor + " mb-1 text-xs"}>{linha.badge}</Badge>
                      <h3 className="font-bold text-foreground">{linha.nome}</h3>
                      <p className="text-xs text-muted-foreground">{linha.subtitulo}</p>
                    </div>
                  </div>
                  <p className="text-sm text-muted-foreground mb-4">{linha.descricao}</p>
                  <div className="grid grid-cols-3 gap-2 mb-4">
                    {[
                      ...
                      <div key={info.label} className="bg-muted rounded-lg p-2 text-center">
                  ...
                  <Button asChild className="w-full bg-[var(--color-caixa-blue)] hover:bg-primary text-primary-foreground font-bold text-sm">
                    <Link href="/simular">
                      Simular
                      <ArrowRight className="ml-2 h-4 w-4" />
                    </Link>
        ...
        <div className="container px-4">
          <div className="max-w-5xl mx-auto">
            <div className="max-w-3xl mb-8">
              <p className="text-sm font-bold uppercase tracking-wider text-primary mb-3">Comparação orientativa</p>
              <h2 className="text-3xl font-bold text-foreground mb-4">Entenda quando cada solução pode fazer sentido</h2>
            ...
            <div className="overflow-x-auto rounded-2xl border border-border shadow-sm">
              <table className="w-full min-w-[760px] text-left">
                <thead className="bg-[#123a5a] text-primary-foreground">
                  <tr>
                ...
                <tbody className="divide-y divide-gray-100 text-sm text-foreground">
                  <tr className="bg-card"><td className="p-4 font-semibold text-foreground">Consignado</td><td className="p-4">Necessidades de uso livre por público elegível</td><td className="p-4">Convênio e margem consignável</td><td className="p-4">CET, prazo e impacto do desconto em folha</td></tr>
                  <tr className="bg-muted"><td className="p-4 font-semibold text-foreground">Crédito pessoal</td><td className="p-4">Despesas planejadas ou necessidade pontual</td><td className="p-4">Custo total e comprometimento de renda</td><td className="p-4">CET, parcela, total a pagar e condições</td></tr>
                  <tr className="bg-card"><td className="p-4 font-semibold text-foreground">Financiamento imobiliário</td><td className="p-4">Aquisição de imóvel elegível</td><td className="p-4">Entrada, avaliação, documentação e longo prazo</td><td className="p-4">Sistema de amortização, CET, seguros e total</td></tr>
                  <tr className="bg-muted"><td className="p-4 font-semibold text-foreground">Financiamento de veículo</td><td className="p-4">Compra de veículo novo ou usado</td><td className="p-4">Entrada, valor do bem e custo do contrato</td><td className="p-4">CET, prazo, parcela e valor total financiado</td></tr>
                </tbody>
              </table>
            </div>
          </div>
        ...
        <div className="container px-4">
          <div className="max-w-5xl mx-auto">
            <div className="text-center max-w-3xl mx-auto mb-10">
              <p className="text-sm font-bold uppercase tracking-wider text-primary mb-3">Do planejamento à proposta</p>
              <h2 className="text-3xl font-bold text-foreground mb-4">Como funciona a análise de crédito</h2>
              <p className="text-muted-foreground">O fluxo pode variar entre instituições, mas normalmente passa pelas etapas abaixo.</p>
            </div>
            <div className="grid md:grid-cols-4 gap-5 mb-12">
              {[
                ...
                <article key={step.title} className="bg-card rounded-2xl border border-border p-6 shadow-sm">
            ...
            <div className="grid md:grid-cols-2 gap-6">
              ...
              <div className="bg-warning/10 rounded-2xl border border-warning/20 p-7">
                <AlertCircle className="h-7 w-7 text-warning mb-4" />
                <h3 className="text-xl font-bold text-foreground mb-4">Cuidados antes de assinar</h3>
                <ul className="space-y-3 text-sm text-foreground">
                  {["Confirme o CET e o valor total a pagar", "Leia regras de atraso, seguros e tarifas", "Verifique datas de vencimento e forma de débito", "Não faça pagamentos antecipados a desconhecidos", "Guarde a proposta e o contrato formalizados"].map((item) => (
                    <li key={item} className="flex gap-2"><CheckCircle2 className="h-5 w-5 text-warning flex-shrink-0" />{item}</li>
                  ))}
                </ul>
              </div>
            </div>
        ...
        <div className="container px-4">
          <div className="max-w-4xl mx-auto">
            <div className="text-center mb-10">
              <p className="text-sm font-bold uppercase tracking-wider text-primary mb-3">Dúvidas frequentes</p>
              <h2 className="text-3xl font-bold text-foreground">Crédito para pessoa física</h2>
            </div>
              ...
              {creditoPessoalFaqs.map((faq) => (
                <details key={faq.question} className="group rounded-xl border border-border bg-card p-5 open:shadow-sm">
                  <summary className="cursor-pointer list-none font-semibold text-foreground flex items-center justify-between gap-4">
                    {faq.question}
                    <span aria-hidden="true" className="text-primary text-xl group-open:rotate-45 transition-transform">+</span>
                  </summary>
      ...
      {/* CTA */}
      <section className="py-14 bg-gradient-to-br from-[#1B4F72] to-[#0d2b45] text-primary-foreground">
        <div className="container px-4">
