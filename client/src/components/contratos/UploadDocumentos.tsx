...
const COR_BADGE: Record<string, string> = {
  ...
  indigo:  'bg-primary/20 text-primary border-primary/20',
  emerald: 'bg-success/20 text-success border-success/20',
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        ...
        {GRUPOS.map(grupo => (
          <div key={grupo}>
            <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-1">{grupo}</p>
      ...
      >
        ...
        <div className="flex flex-col items-center gap-2">
          <div className={`w-10 h-10 rounded-full flex items-center justify-center ${dragOver ? 'bg-primary/20' : 'bg-muted'}`}>
              ...
              >
                ...
                <div className="flex-shrink-0">
                  {doc.tipo === 'imagem' && doc.previewUrl ? (
                    ...
                    <div className={`w-16 h-16 rounded-lg border flex flex-col items-center justify-center gap-1 ${COR_PREVIEW[cat.cor] ?? 'bg-muted border-border'}`}>
                      <div className={COR_TEXT[cat.cor] ?? 'text-muted-foreground'}>
                        {cat.icon ? <span className="scale-150 block">{cat.icon}</span> : <FileText className="w-6 h-6" />}
                      </div>
                      <span className={`text-[9px] font-bold uppercase ${COR_TEXT[cat.cor] ?? 'text-muted-foreground'}`}>PDF</span>
                    </div>
                  )}
                </div>
                ...
                <div className="flex-1 min-w-0 space-y-1.5">
                  ...
                  <input
                    ...
                    className="w-full text-xs border border-border rounded-lg px-2.5 py-1.5 focus:outline-none focus:ring-1 focus:ring-blue-400"
                    disabled={disabled}
                  />
                  ...
                  <select
                    ...
                    className="w-full text-xs border border-border rounded-lg px-2.5 py-1.5 focus:outline-none focus:ring-1 focus:ring-blue-400 bg-card"
                    disabled={disabled}
                  >
                    {GRUPOS.map(g => (
                  ...
                  <label className="flex items-center gap-1.5 text-[11px] font-semibold text-muted-foreground cursor-pointer select-none">
                    <input
                      ...
                      disabled={disabled}
                      className="h-3.5 w-3.5 rounded border-input text-primary focus:ring-blue-400"
                    />
                ...
                <button
                  ...
                  disabled={disabled}
                  className="flex-shrink-0 mt-1 p-1 rounded-lg text-muted-foreground hover:text-destructive hover:bg-destructive/10 transition-colors"
                >
                  <X className="w-4 h-4" />
                </button>
              </div>
