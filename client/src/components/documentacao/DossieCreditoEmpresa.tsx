    ...
    <section className={`rounded-2xl border p-4 ${apto ? "border-success/20 bg-success/10" : falhasLeitura.length ? "border-destructive/20 bg-destructive/10/40" : "border-warning/20 bg-warning/10/50"}`}>
      ...
      <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-3">
        {documentos.map((item) => {
          ...
          return (
            <article key={item.codigo || item.nome} className="rounded-xl border border-white/90 bg-card p-3 shadow-sm">
      ...
      <div className="mt-3 grid grid-cols-2 gap-2 md:grid-cols-4">
        <div className="rounded-xl border border-white/80 bg-card p-3"><span className="text-[10px] font-bold uppercase text-muted-foreground">Analisados</span><b className="block text-base text-foreground">{identidade.relatorio?.documentos_analisados ?? documentos.filter((item) => item.analisado).length}/{totalDocumentos}</b></div>
        <div className="rounded-xl border border-white/80 bg-card p-3"><span className="text-[10px] font-bold uppercase text-muted-foreground">Consistentes</span><b className="block text-base text-foreground">{identidade.relatorio?.documentos_conferidos ?? 0}/{totalDocumentos}</b></div>
        <div className="rounded-xl border border-white/80 bg-card p-3"><span className="text-[10px] font-bold uppercase text-muted-foreground">Tempo de abertura</span><b className="block text-base text-foreground">{identidade.idade_meses == null ? "Não confirmado" : `${identidade.idade_meses} meses`}</b></div>
        <div className="rounded-xl border border-white/80 bg-card p-3"><span className="text-[10px] font-bold uppercase text-muted-foreground">Enquadramento</span><b className="block truncate text-sm text-foreground">{identidade.enquadramento_tributario || "Não identificado"}</b></div>
      </div>
  ...
  return (
    <section className={`rounded-2xl border p-4 ${apto ? "border-success/20 bg-success/10" : "border-primary/20 bg-primary/10/50"}`}>
      ...
      <div className="mt-4 grid grid-cols-2 gap-3 lg:grid-cols-4">
        <div className="rounded-xl border border-white bg-card p-3"><p className="text-[10px] font-bold uppercase text-muted-foreground">Atos da Junta</p><p className="mt-1 text-xs font-extrabold text-foreground">{dados.atos_junta_aprovados ? "Analisado e aprovado" : dados.atos_junta_anexados ? "Aguardando análise" : "Não anexado"}</p></div>
        <div className="rounded-xl border border-white bg-card p-3"><p className="text-[10px] font-bold uppercase text-muted-foreground">Contratos/alterações</p><p className="mt-1 text-xs font-extrabold text-foreground">{dados.total_contratos_anexados || 0} anexado(s)</p></div>
        <div className="rounded-xl border border-white bg-card p-3"><p className="text-[10px] font-bold uppercase text-muted-foreground">Último registro</p><p className="mt-1 text-xs font-extrabold text-foreground">{formatDate(dados.ultimo_registro_junta?.data || undefined)}</p></div>
        <div className="rounded-xl border border-white bg-card p-3"><p className="text-[10px] font-bold uppercase text-muted-foreground">Corte mínimo</p><p className="mt-1 text-xs font-extrabold text-foreground">{formatDate(dados.data_corte_12_meses || undefined)}</p></div>
      </div>
              ...
              {!!etapa.documentos.length && (
                <div className="mt-2 space-y-1.5">
                  {etapa.documentos.map((documento) => (
                    <div key={documento.codigo} className="flex items-start gap-2 rounded-lg border border-white bg-card p-2">
