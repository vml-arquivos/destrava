    ...
    <div className="space-y-4">
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-4 space-y-3">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-2.5">
          <div>
            <p className="text-sm font-bold text-slate-700 flex items-center gap-1.5"><FolderOpen className="w-4 h-4" /> Documentação anexada</p>
            <p className="text-[11px] text-slate-400 mt-0.5">Visualize os documentos anexados, pesquise, filtre ou exporte em checklist. Os arquivos físicos seguem preservados mesmo com atualizações do cadastro.</p>
          </div>
          <div className="flex flex-col sm:flex-row gap-2">
            <div className="relative">
              <Search className="w-3.5 h-3.5 absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" />
              <input value={busca} onChange={(e) => setBusca(e.target.value)} placeholder="Buscar documento..." className="h-9 w-full sm:w-56 rounded-lg border border-slate-200 bg-white pl-9 pr-3 text-xs text-slate-700" />
            </div>
        ...
        {loading ? (
          <div className="flex items-center justify-center py-10 text-xs text-slate-500"><Loader2 className="w-4 h-4 animate-spin mr-2" /> Carregando documentos...</div>
        ) : docs.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-14 gap-3 rounded-xl border-2 border-dashed border-slate-200 bg-slate-50/60">
            ...
            <div className="text-center max-w-xs">
              ...
              <p className="text-xs text-slate-400 mt-1">Use o checklist logo abaixo para anexar cada documento no campo certo.</p>
            </div>
          </div>
        ) : docsFiltrados.length === 0 ? (
          <div className="rounded-xl border border-slate-100 bg-slate-50 p-4 text-xs text-slate-500">Nenhum documento encontrado para o filtro aplicado.</div>
        ) : (
          <div className="space-y-3">
            ...
            {secoesDaTela.map((secao) => {
              ...
              return (
                <div key={secao.titulo} className="rounded-lg border border-slate-100 overflow-hidden self-start">
                  <div className="px-3 py-2 bg-slate-50/80 border-b border-slate-100 flex items-center justify-between gap-2">
                    <p className="text-[11px] font-bold text-slate-600 uppercase tracking-wide">{secao.titulo}</p>
                    <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-full bg-white border border-slate-200 text-slate-500">{docsSecao.length}</span>
                  </div>
                  <div className="divide-y divide-slate-100">
                    {docsSecao.map((doc) => (
                      <div key={doc.id} className="flex items-center gap-2.5 px-3 py-2.5 bg-white hover:bg-slate-50/80 transition-colors group">
                        {/* Ícone compacto */}
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-1.5 flex-wrap">
                            <p className="text-xs font-semibold text-slate-800 truncate max-w-[220px]">{doc.nome_customizado || doc.nome_original}</p>
                            <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full border ${statusCls[doc.status] || "bg-slate-50 text-slate-600 border-slate-100"}`}>{doc.status.replace(/_/g, " ")}</span>
                            {doc.validado && <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-100 flex items-center gap-0.5"><CheckCircle className="w-2.5 h-2.5" />ok</span>}
                          </div>
                          ...
                          {doc.observacoes && <p className="text-[10px] text-slate-500 truncate">{doc.observacoes}</p>}
                        </div>
                        {/* Botões de ação — aparecem no hover */}
                        <div className="flex items-center gap-0.5 shrink-0 opacity-60 group-hover:opacity-100 transition-opacity">
                          <button title="Visualizar" onClick={() => visualizar(doc)} className="p-1.5 rounded-md hover:bg-blue-50 text-blue-600 transition-colors"><Eye className="w-3.5 h-3.5" /></button>
                          <button title="Baixar" onClick={() => baixar(doc)} className="p-1.5 rounded-md hover:bg-slate-100 text-slate-500 transition-colors"><Download className="w-3.5 h-3.5" /></button>
                          <button title="Imprimir" onClick={() => imprimir(doc)} className="p-1.5 rounded-md hover:bg-slate-100 text-slate-500 transition-colors"><Printer className="w-3.5 h-3.5" /></button>
                          {permitirValidar && (
                            <button onClick={() => validar(doc.id, !doc.validado)} title={doc.validado ? "Reabrir" : "Validar"} className={`p-1.5 rounded-md text-[10px] font-bold transition-colors ${doc.validado ? "hover:bg-amber-50 text-amber-600" : "hover:bg-emerald-50 text-emerald-600"}`}>
                              {doc.validado ? "↩" : "✓"}
                            </button>
                          )}
                          {permitirExcluir && <button title="Excluir" onClick={() => excluir(doc.id)} className="p-1.5 rounded-md hover:bg-red-50 text-red-500 transition-colors"><Trash2 className="w-3.5 h-3.5" /></button>}
                        </div>
                      </div>
                    ))}
                  </div>
