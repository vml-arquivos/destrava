    ...
    <div className="mt-1.5 rounded-lg border border-border bg-muted p-2 space-y-1.5">
      ...
      {!!badges.length && (
        <div className="grid grid-cols-2 gap-1">
          {badges.map((item) => (
            <div key={item.label} className="rounded border border-white bg-card px-1.5 py-1">
      ...
      {entidadeTipo === "empresa" && empresaId && relatorioDocumental && relatorioModalAberto && (
        <div className="fixed inset-0 z-50 bg-overlay p-4 flex items-center justify-center">
          ...
          <div className="grid grid-cols-2 gap-2 md:grid-cols-5">
            <div className="rounded-xl border border-white bg-card p-2.5"><p className="text-[9px] font-black uppercase text-muted-foreground">Status geral</p><p className="mt-1 text-[11px] font-black text-foreground">{relatorioDocumental.status_geral}</p></div>
            <div className="rounded-xl border border-white bg-card p-2.5"><p className="text-[9px] font-black uppercase text-muted-foreground">Anexados e analisados</p><p className="mt-1 text-lg font-black text-success">{relatorioDocumental.resumo?.documentos_analisados ?? 0}</p></div>
            <div className="rounded-xl border border-white bg-card p-2.5"><p className="text-[9px] font-black uppercase text-muted-foreground">Anexados aguardando análise</p><p className="mt-1 text-lg font-black text-warning">{relatorioDocumental.resumo?.documentos_pendentes_analise ?? 0}</p></div>
            <div className="rounded-xl border border-white bg-card p-2.5"><p className="text-[9px] font-black uppercase text-muted-foreground">Ainda faltam anexar</p><p className="mt-1 text-lg font-black text-warning">{relatorioDocumental.resumo?.documentos_faltantes ?? 0}</p></div>
            <div className="rounded-xl border border-white bg-card p-2.5"><p className="text-[9px] font-black uppercase text-muted-foreground">Blocos com registro</p><p className="mt-1 text-lg font-black text-foreground">{relatorioDocumental.resumo?.blocos_analisados ?? 0}</p></div>
          </div>
              ...
              <div className="rounded-2xl border border-primary/20 bg-primary/10 p-3">
                <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                  ...
                  <div className="flex flex-wrap items-center justify-end gap-2 text-[11px]">
                    <span className="rounded-lg border border-white bg-card px-2.5 py-1.5 font-semibold text-muted-foreground"><b className="text-foreground">{docs.length}</b> arquivos</span>
                    <span className="rounded-lg border border-white bg-card px-2.5 py-1.5 font-semibold text-muted-foreground"><b className="text-success">{documentosValidados}</b> validados</span>
          ...
          {secaoAtivaTitulo === "Documentação da Empresa" && entidadeTipo === "empresa" && societaria?.habilitada && (() => {
            ...
            return (
              <div className={`rounded-2xl border p-3 ${apto ? "border-success/20 bg-success/10" : "border-warning/20 bg-warning/10"}`}>
                ...
                {registros.length > 0 && (
                  <div className="mt-3 rounded-xl border border-white bg-card p-2.5">
