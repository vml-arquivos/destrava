                ...
                return (
                  <div key={`${documento.codigo}-${index}`} className="rounded-xl border border-emerald-200 bg-white p-3">
                    ...
                    {Array.isArray(resultado.alteracoes_societarias) && resultado.alteracoes_societarias.length > 0 && <div className="mt-2 rounded-lg border border-indigo-200 bg-indigo-50/60 p-2"><p className="text-[9px] font-black uppercase text-indigo-800">Alteração societária identificada</p>{resultado.alteracoes_societarias.map((alteracao: any, alteracaoIndex: number) => { const cedente = alteracao?.cedente?.nome || alteracao?.socio_retirante?.nome || "cedente não identificado"; const cessionario = alteracao?.cessionario?.nome || alteracao?.socio_admitido?.nome || "cessionário não identificado"; const quotas = alteracao?.quotas_transferidas ?? alteracao?.cedente?.quotas; return <div key={alteracaoIndex} className="mt-2 rounded-lg border border-indigo-100 bg-white p-2"><p className="text-[10px] font-semibold text-slate-800">Retirada/cedente: <span className="font-black">{cedente}</span></p><p className="mt-1 text-[10px] font-semibold text-slate-800">Entrada/cessionário: <span className="font-black">{cessionario}</span></p><p className="mt-1 text-[10px] text-slate-700">Quotas transferidas: <span className="font-semibold">{quotas ?? "não identificado"}</span>{alteracao?.percentual_transferido != null ? ` (${alteracao.percentual_transferido}%)` : ""}</p>{alteracao?.clausula && <p className="mt-1 text-[10px] text-slate-700">Cláusula: {alteracao.clausula}</p>}{alteracao?.pagina && <p className="mt-1 text-[10px] text-slate-500">Página: {alteracao.pagina}</p>}{alteracao?.evidencia && <p className="mt-1 whitespace-pre-line text-[10px] italic text-slate-600">Evidência: “{alteracao.evidencia}”</p>}</div>; })}</div>}
                     ...
                     {!!Object.keys(agenteSocietario).length && <div className="mt-2 rounded-lg border border-violet-200 bg-violet-50/70 p-2"><div className="flex flex-wrap items-center justify-between gap-2"><p className="text-[9px] font-black uppercase text-violet-800">Agente societário auditável</p><span className={`rounded-full px-2 py-0.5 text-[9px] font-black ${agenteSocietario.revisao_obrigatoria ? "bg-amber-100 text-amber-800" : "bg-violet-100 text-violet-800"}`}>{agenteSocietario.revisao_obrigatoria ? "Revisão humana obrigatória" : "Leitura estruturada concluída"}</span></div>{agenteSocietario.ato_praticado && <p className="mt-1 text-[10px] font-semibold text-violet-950"><strong>Ato praticado:</strong> {agenteSocietario.ato_praticado}</p>}{agenteSocietario.estado_atual?.descricao && <p className="mt-1 text-[10px] text-slate-700"><strong>Estado atual:</strong> {agenteSocietario.estado_atual.descricao}</p>}{agenteSocietario.confronto_qsa?.status && <p className="mt-1 text-[10px] text-slate-700"><strong>Confronto com QSA:</strong> {agenteSocietario.confronto_qsa.status}{agenteSocietario.confronto_qsa.mensagem ? ` — ${agenteSocietario.confronto_qsa.mensagem}` : ""}</p>}{resultado.qsa_adicional_necessario === true ? <div className="mt-2 rounded-lg border border-amber-200 bg-amber-50 p-2 text-[10px] text-amber-900"><strong>QSA adicional necessário:</strong> {resultado.qsa_adicional_motivo || "O quadro da última alteração vigente possui sócio não localizado no QSA atual."}</div> : agenteSocietario.status_documento === "atual" && agenteSocietario.confronto_qsa?.status === "confirmado" ? <div className="mt-2 rounded-lg border border-emerald-200 bg-emerald-50 p-2 text-[10px] text-emerald-900"><strong>QSA vigente confirmado:</strong> a última alteração/contrato vigente define o quadro atual e não é necessário solicitar outro QSA.</div> : null}{Array.isArray(agenteSocietario.linha_tempo_societaria) && agenteSocietario.linha_tempo_societaria.length > 0 && <div className="mt-2"><p className="text-[9px] font-black uppercase text-violet-700">Linha do tempo societária</p>{agenteSocietario.linha_tempo_societaria.map((evento: any, eventoIndex: number) => <p key={eventoIndex} className="mt-1 text-[10px] text-slate-700">• {evento?.data || "Data não identificada"} — {evento?.tipo_ato || "Ato societário"}{evento?.numero_arquivamento ? ` — registro ${evento.numero_arquivamento}` : ""}</p>)}</div>}{Array.isArray(agenteSocietario.motivos_revisao) && agenteSocietario.motivos_revisao.length > 0 && <div className="mt-2 rounded-lg border border-amber-200 bg-amber-50 p-2"><p className="text-[9px] font-black uppercase text-amber-800">Motivos para revisão</p>{itensTextoRelatorio(agenteSocietario.motivos_revisao).map((motivo, motivoIndex) => <p key={motivoIndex} className="mt-1 text-[10px] text-amber-900">• {motivo}</p>)}</div>}</div>}
