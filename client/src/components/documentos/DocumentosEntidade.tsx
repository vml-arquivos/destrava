...
const CAMPO_ANALISE_LABEL: Record<string, string> = {
  ...
  vinculo_cnpj: "Vínculo com o CNPJ",
  quantidade_integrantes: "Quantidade de integrantes",
  administrador_titular: "Administrador/Titular",
  resultado_qsa: "Resultado",
...
export default function DocumentosEntidade({
  ...
  const modulosRelatorio = Array.isArray(relatorioDocumental?.modulos_relatorio)
    ? [...relatorioDocumental.modulos_relatorio].sort((a: any, b: any) => Number(a.ordem || 0) - Number(b.ordem || 0))
    : [];
      ...
      {entidadeTipo === "empresa" && empresaId && relatorioDocumental && relatorioModalAberto && (
        <div className="fixed inset-0 z-50 bg-overlay p-4 flex items-center justify-center">
          ...
          <div className="rounded-xl border border-primary/25 bg-primary/5 p-3">
            <p className="text-xs font-black text-primary">Checklist e análise documental</p>
            <p className="mt-1 text-[10px] text-muted-foreground">Os documentos aparecem uma única vez, agrupados por identidade, documentos principais, consultas da empresa, documentação dos sócios, consultas dos sócios e pendências.</p>
          </div>

          {modulosRelatorio.length > 0 && <div className="rounded-xl border border-primary/25 bg-card p-3">
            <div className="flex flex-col gap-1 md:flex-row md:items-start md:justify-between">
              <div><p className="text-xs font-black text-primary">Checklist e análise documental</p><p className="mt-1 text-[10px] text-muted-foreground">Ordem institucional: identidade da empresa, documentos principais, consultas da empresa, documentação dos sócios, consultas dos sócios e pendências. O conteúdo interno da assessoria não aparece neste modo.</p></div>
              <span className="rounded-full bg-primary/10 px-2 py-1 text-[9px] font-black text-primary">{relatorioDocumental.modo_relatorio === "interno" ? "Interno autorizado" : "Institucional"}</span>
            </div>
            <nav className="mt-3 rounded-lg border border-border bg-muted/30 p-2"><p className="text-[9px] font-black uppercase text-muted-foreground">Índice</p><ol className="mt-1 grid gap-1 md:grid-cols-2">{modulosRelatorio.map((modulo: any) => <li key={modulo.id} className="text-[10px]"><a href={`#relatorio-modulo-${modulo.id}`} className="font-semibold text-primary hover:underline">{modulo.ordem}. {modulo.titulo}</a></li>)}</ol></nav>
            <div className="mt-3 space-y-3">{modulosRelatorio.map((modulo: any) => <section key={modulo.id} id={`relatorio-modulo-${modulo.id}`} className="scroll-mt-3 rounded-lg border border-border bg-background p-3">
              <div className="flex items-start gap-2"><span className="inline-flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-primary text-[10px] font-black text-primary-foreground">{modulo.ordem}</span><div><p className="text-[11px] font-black text-foreground">{modulo.titulo}</p><p className="mt-0.5 text-[10px] text-muted-foreground">{modulo.descricao}</p></div></div>
              {Array.isArray(modulo.campos) && modulo.campos.length > 0 && <div className="mt-2 grid gap-2 md:grid-cols-3">{modulo.campos.map((campo: any, index: number) => <div key={`${campo.campo}-${index}`} className="rounded-md border border-border bg-card p-2"><p className="text-[9px] font-black uppercase text-muted-foreground">{campo.campo}</p><p className="mt-1 text-[10px] font-semibold text-foreground">{campo.valor || "Não informado"}</p><p className="mt-1 text-[9px] text-muted-foreground">{campo.status || "não confirmado"} · {campo.fontes?.join(", ") || "fonte não localizada"}</p></div>)}</div>}
              {Array.isArray(modulo.socios) && modulo.socios.length > 0 && <div className="mt-2 grid gap-2 md:grid-cols-2">{modulo.socios.map((socio: any, index: number) => <div key={`${socio.id || socio.cpf || socio.nome}-${index}`} className="rounded-md border border-border bg-card p-2"><p className="text-[10px] font-black">{socio.nome || socio.nome_completo || "Sócio não identificado"}</p><p className="text-[9px] text-muted-foreground">{socio.cpf || socio.documento || "Identificador não localizado"} · {socio.cargo || socio.qualificacao || "cargo não localizado"}</p><p className="text-[9px] text-muted-foreground">Entrada: {socio.data_entrada || "não localizada"} · Status: {socio.status || "ativo"}</p></div>)}</div>}
              {Array.isArray(modulo.submodulos) && modulo.submodulos.length > 0 ? <div className="mt-2 grid gap-2 md:grid-cols-2">{modulo.submodulos.map((submodulo: any) => <div key={submodulo.id} className="rounded-md border border-border bg-card p-2"><p className="text-[10px] font-black text-primary">{submodulo.titulo}</p><p className="mt-1 text-[9px] text-muted-foreground">{Array.isArray(submodulo.itens) ? `${submodulo.itens.length} documento(s)` : "Nenhum documento"}</p></div>)}</div> : null}
              {Array.isArray(modulo.itens) && modulo.itens.length > 0 && <div className="mt-2 space-y-2">{modulo.itens.map((item: any, index: number) => <div key={`${item.arquivo_id || item.nome}-${index}`} className="rounded-md border border-border bg-card p-2"><div className="flex items-start justify-between gap-2"><p className="text-[10px] font-black">{item.nome || "Documento"}</p><span className={`rounded-full px-2 py-0.5 text-[9px] font-black ${/confirm|aprov|valid/i.test(String(item.status_validacao || item.status)) ? "bg-success/15 text-success" : /incompat|diverg|revis/i.test(String(item.status_validacao || item.status)) ? "bg-destructive/10 text-destructive" : "bg-warning/15 text-warning"}`}>{item.status_validacao || item.status || "Não informado"}</span></div><p className="mt-1 text-[9px] text-muted-foreground">Datas disponíveis: {Array.isArray(item.datas) && item.datas.length ? item.datas.join(", ") : (item.data || "não informadas")} · Validade/situação: {item.validade || item.status_validacao || "não informada"} · Arquivo original: {Array.isArray(item.arquivos_originais) && item.arquivos_originais.length ? item.arquivos_originais.join(", ") : (item.arquivo_original || "não localizado")}</p>{item.resultado ? <p className="mt-1 text-[10px]">{item.resultado}</p> : null}{item.pendencia ? <p className="mt-1 text-[10px] font-semibold text-warning">{item.pendencia}</p> : null}</div>)}</div>}
              {Array.isArray(modulo.eventos) && modulo.eventos.length > 0 && <div className="mt-2 space-y-1">{modulo.eventos.map((evento: any, index: number) => <p key={`${evento.data}-${evento.numero_arquivamento}-${index}`} className="text-[10px]"><strong>{evento.data || "Data não localizada"}</strong> — {evento.tipo_ato || "Ato"} — {evento.mudanca || evento.impacto || "Alteração não descrita"}</p>)}</div>}
              {!modulo.incluida && <p className="mt-2 rounded-md border border-dashed border-border bg-muted/30 p-2 text-[10px] text-muted-foreground">Este módulo é interno e foi omitido do relatório institucional.</p>}
            </section>)}</div>
          </div>}
                          ...
                          <div className="rounded-md border border-border bg-muted p-2">
                            ...
                            <div className={camposExpandidos[chaveSlot] && docsTipo.length > 3 ? "space-y-1 sm:max-h-44 sm:overflow-y-auto sm:pr-1" : "space-y-1"}>
                              {(camposExpandidos[chaveSlot] ? docsTipo : docsTipo.slice(0, 3)).map((doc) => {
                                ...
                                // recolhido por padrão inclusive em incompatibilidade/revisão. Isso
                                // preserva o alerta sem voltar a alongar toda a grade documental.
