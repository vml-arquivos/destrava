# Relatório de Build — 31/08/2026 (atualizado, Rodada 6 — Enquadramento Tributário duplicado no relatório)
...
## 3. Suíte de testes
`npx vitest run` -- 79 arquivos / 727 testes, todos passando (ver `TEST_REPORT.md` para a progressão completa entre as seis rodadas desta sessão).

## 4. Build de produção
`pnpm run build` -- concluído com sucesso (executado e confirmado novamente após a conclusão da mudança desta rodada: correção do regex de deduplicação em `chaveDocumentoRelatorio`, `server/routes/documentacao.ts`).
