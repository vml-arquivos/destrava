...
- `dist/index.js` 2.5 MB / `dist/backfill-laudos.js` 473.2 kB -- `node --check` OK em ambos.

### 4. O que mudou
Usuário enviou 2 prints da tela de produção (ainda no código antigo, pré-deploy da parte 18) e uma mensagem de voz com vários pedidos: confronto de identidade usando "qualquer os documentos, os dados que tenha na empresa" (não só o Contrato Social, esclarecido por pergunta de acompanhamento), harmonização da página de documentos ("totalmente desformatada, desconfigurada, botão por cima de nomes"), e reforço do pedido -- deliberadamente adiado na parte 18 -- de um seletor de tipo de documento na tela de upload. Três frentes:

**(1) Identidade do sócio.** A validação introduzida na parte 18 (`validarIdentidadeSocioExtraida`) só cruzava contra o QSA sincronizado, que tipicamente não traz o CPF do sócio (confirmado por leitura de código) -- deixando o lado do CPF praticamente inerte. Nova consulta `carregarDocumentosAnterioresDoSocio` (`analiseDocumentalEspecializada.ts`) busca o laudo mais recente de cada OUTRO documento já enviado/analisado para o MESMO `socio_id`, e usa o CPF/nome já estabelecido por um documento anterior confiável (não ele mesmo divergente) como fonte de apoio quando o QSA não tem o dado -- QSA continua tendo prioridade quando disponível; CPFs/nomes conflitantes entre documentos anteriores ficam inconclusivos; falha na consulta nunca derruba a análise principal.

**(2) Layout.** `<p>` do nome do arquivo, no card de cada documento do Acervo Documental, é item de um container flex e usa `truncate` (que aplica `white-space: nowrap`) sem `min-w-0` próprio -- min-content de um item flex em `nowrap` é a largura total do texto, então `truncate` nunca entrava em ação de verdade, e o nome vazava visualmente sob a coluna de botões em nomes de arquivo mais longos. Corrigido com `min-w-0` direto no `<p>`; auditados todos os demais usos de `truncate` do mesmo arquivo e de `AcervoDocumentalWorkspace.tsx`, nenhum outro com o mesmo problema. Sem acesso à sessão autenticada do usuário para reproduzir ao vivo -- correção feita por leitura cuidadosa do CSS/flexbox, recomenda-se confirmação visual do usuário após o deploy.

**(3) Seletor de tipo + Passaporte de ponta a ponta.** Seletor opcional (nunca bloqueia o anexo) ao lado do botão "Anexar" do campo de identidade do sócio, com RG/CNH/Passaporte/CPF/"Não sei ou outro" (padrão). Ao implementar, confirmado que Passaporte nunca tinha ponta a ponta completa apesar de já ter leitor local desde a parte 18: faltava entrada no catálogo (`shared/documentTypes.ts`), faltava no marcador do classificador central (só reconhecia RG/CPF/CNH como identidade válida), e faltava em 6 outras listas de tipos equivalentes espalhadas pelo backend/frontend -- todas corrigidas.

## Rodada 11/09/2026 parte 18 — Seis causas raiz adicionais de leitura documental (CNH/comprovante/Declaração REAIS); confronto de identidade contra o QSA; Passaporte

Base: mesma árvore da parte 17 (mesma entrega), `node_modules` já instalado nesta verificação (sem alteração de dependências).

### 1. Typecheck
`npx tsc --noEmit` -- concluído sem nenhum erro.

### 2. Suíte de testes
`npx vitest run` -- **131 arquivos / 1218 testes, todos passando** (1201 + 17 novos em `tests/leituraDocumentalDinamicaMultiSocio.test.ts`).

### 3. Build de produção
`pnpm run build` -- concluído com sucesso.
- JavaScript inicial: 98.8 kB gzip (limite 130 kB) -- OK
- CSS inicial: 31.4 kB gzip (limite 45 kB) -- OK
- `dist/index.js` 2.5 MB / `dist/backfill-laudos.js` 467.3 kB -- `node --check` OK em ambos.

### 4. O que mudou
Com 6 arquivos reais (SPC/Serasa, Declaração e Recibo de IRPF, comprovante de residência de telecom, CNH em PDF "CNH Digital" sem camada de texto, e um snapshot do repositório com o Pull Request #81 aberto e travado por conflito) e 5 prints, o usuário mostrou que a leitura documental ainda falhava mesmo após a parte 17, e pediu confronto de identidade contra o QSA e um subtipo explícito de documento (RG/CNH/Passaporte). Auditado o PR #81: seu desenho de confronto de identidade (`validarIdentidadeSocioExtraida`) foi adotado, com um reforço (CPF em formato estrito antes de "todos os dígitos", evitando a mesma poluição de coluna corrigida abaixo). Com os documentos reais, seis causas raiz adicionais foram confirmadas e corrigidas: (1) DPI de OCR (300, herdado da parte 17) insuficiente para os campos CPF/Nº Registro de uma CNH sem camada de texto -- elevado para 400, confirmado empiricamente com a mesma página real em 6 resoluções distintas; (2) nome do titular saindo colado ao campo vizinho pelo próprio OCR -- novo helper `primeiroTrechoDeNomeAntesDeRuido`; (3) endereço do comprovante puxado do EMISSOR da fatura (campo padrão "Endereço do Beneficiário" do boleto anexado), não do titular -- linhas de Beneficiário/Cedente/Sacador/Favorecido agora excluídas da busca; (4) CEP confundido com Inscrição Estadual (varredura cega por 8 dígitos) -- agora prioriza um CEP explicitamente rotulado; (5) classificador central confundindo o campo padrão FEBRABAN "Sacador/Avalista" (presente em praticamente todo boleto brasileiro) com um instrumento de Aval -- regra ajustada para ignorar essa combinação específica sem deixar de reconhecer um aval de verdade; (6) nenhum documento de identidade/IRPF era cruzado contra o sócio (QSA) do card de upload -- `validarIdentidadeSocioExtraida` agora confronta e sinaliza divergência sem redirecionar automaticamente. Passaporte adicionado como quarto subtipo de documento de identidade (reaproveita o slot `documento_socio`). Diagnóstico da leitura (extração parcial / OCR) agora exposto como alerta genérico, reaproveitando a renderização de alertas já existente -- zero código novo de frontend. Verificado de ponta a ponta com os 4 documentos reais anexados nesta rodada, todos chegando a `DADO_COMPROVADO` com identidade confirmada contra o QSA.

**Decisão de escopo, comunicada ao usuário:** o seletor explícito de tipo na tela de upload (RG/Passaporte/CNH) foi propositalmente adiado -- com as seis causas raiz corrigidas, os quatro subtipos já são reconhecidos automaticamente pelo backend, resolvendo o sintoma relatado; segue como melhoria de UX para uma rodada futura.

**Nota para o usuário, fora do escopo de código desta rodada:** assim como observado na parte 15, as variáveis de ambiente `DOCUMENT_EXTERNAL_AI_FALLBACK_ENABLED` e `GEMINI_DOCUMENT_OCR_ENABLED` controlam o fallback de leitura por IA externa em produção -- não foi possível confirmar o valor real dessas variáveis a partir desta sessão. `SPC_SERASA_CPF_PALUMA 1.pdf` (também anexado) não foi investigado -- pertence à funcionalidade já existente de Rating/CPF, sem relação com esta rodada.
